...
* def metric to score different variants: MSE;
start compute_mse(y, x, x1, x2);
	n = nrow(x);
	xa = J(n,1,0);
	xb = J(n,1,0);
	
	do i=1 to n;
		if x[i] > x1 then xa[i] = (x[i]-x1);
		if x[i] > x2 then xb[i] = (x[i]-x2);
	end;
...
start optimize_breakpoints(y, x);
	*\
	...
	PARAMETERS:
		y: data
		x: data
		loss: function to use as comparison
		k: the number of observations required above and below and inbetween the breakpoints
		
		Add later:
		b: breaks, the number of breakpoints
	;
	n = nrow(x);
	b = 2;
	k =3;
